	...
	static class ConvoM
	{
		...
		public class BitmapFilter
		{
            ...
            public static bool GaussianBlur(Bitmap b, int nWeight = 4)
            {
                ...
                // A 3x3 Gaussian kernel with a default weight
                m.TopLeft = 1; m.TopMid = 2; m.TopRight = 1;
                m.MidLeft = 2; m.Pixel = 4; m.MidRight = 2;
                m.BottomLeft = 1; m.BottomMid = 2; m.BottomRight = 1;

                // The factor is the sum of all elements in the kernel
                m.Factor = 16;  // Sum of the Gaussian kernel: 1 + 2 + 1 + 2 + 4 + 2 + 1 + 2 + 1

                // Optionally adjust the kernel by the specified weight
                // This allows the blur effect to be stronger or weaker based on the nWeight parameter
                if (nWeight != 4)
                {
                    m.TopLeft *= nWeight;
                    m.TopMid *= nWeight;
